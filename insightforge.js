// InsightForge 웹 애플리케이션 - 완전히 새로운 파일명
// Version: 5.0.0 - ULTIMATE CACHE BUST - NO LOCALHOST
...
async function fetchAPI(url) {
    try {
        ...
        const cacheBuster = `?v=5.0.0&t=${timestamp}&r=${random}&cache=busted&force=reload&no-cache=true&new=file`;
        const finalUrl = url + cacheBuster;
        
        console.log('🚀 API 호출 (ULTIMATE CACHE BUST):', finalUrl);
...
async function loadSidoData() {
    try {
        console.log('📊 시도 데이터 로드 중... (v5.0.0 - ULTIMATE CACHE BUST)');
...
// 초기화
async function init() {
    console.log('🚀 InsightForge 웹 애플리케이션 시작 (v5.0.0 - ULTIMATE CACHE BUST)');
    console.log('✅ localhost:3002 완전 제거됨');
    console.log('✅ Vercel API 사용');
    console.log('✅ 완전히 새로운 파일명 사용');
